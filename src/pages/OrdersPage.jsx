...
function OrderCard({ order, mode, onConfirm }) {
  ...
  const handleWhatsApp = () => {
    const contact = mode === 'buying' ? order.products?.whatsapp_number : order.buyer?.whatsapp_number;
    if (!contact) return;
    const number = contact.replace(/\D/g, '').replace(/^0/, '234');
    const msg = encodeURIComponent(mode === 'buying'
      ? `Hi! I paid for *${order.products?.name}* on CampusPlug. Let's arrange pickup!`
      : `Hi! Your payment for *${order.products?.name}* on CampusPlug has been confirmed.`
    );
